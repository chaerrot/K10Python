...
# for문이 딱 한 번만 실행된 후 탈출하게 되므로 else는 실행되지 않는다.
for i in range(3):
    print(i, end=" ")
    break
else:
...
# 전체 누적 날짜 수
total_days = 0
# 월별 날짜 수를 리스트로 정의 (1월->31일, 7월8월->31일, 12월->31일)
year_month_days = [0,31,28,31,30,31,30,31,31,30,31,30,31]
# 서기 1년부터 입력한 년도까지의 누적 날짜 수를 더한다.
for d in range(1, year):
    if d % 400 == 0: # 400으로 나누어 떨어지면 윤년
        ...
        total_days = total_days + 366
    else: # 그 외에는 모두 평년이므로 365일로 계산함
        total_days = total_days + 365

# 입력 년도의 각 달의 날짜 수를 누적해서 더함
