...
인스턴스 메서드
    : 인스턴스 변수에 항상 접근할 수 있도록 메서드의 첫 번째
...
# 각 객체의 settrt() 메서드를 통해 인스턴스 변수를 초기화한다.
a.setdata(4, 2)
b.setdata(3, 8)
...
클래스메소드: 정적메소드와 비슷한데, 객체 인스턴스를 의미하는
    self대신 cls라는 클래스를 의미하는 파라미터를 전달 받는다.
    cls를 통해 클래스 변수에 액세스할 수 있다.
    @classmethod 라는 데코레이터를 사용한다.
'''
class CalculatorInit:
    #클래스 변수(클래스 전체에서 접근 가능한 전역변수)
    count = 0
    ...
    def __init__(self):
        ...
        self.second = 2 
    '''
    # 생성자 메서드 정의. 멤버변수 초기화를 위해 첫 번째 파라미터에 self를 기술해야 한다.
...
#fCal = FourCalculatorInit() => 인자가 없는 기본생성자는 정의되지 않음.
# 2개의 인자를 전달해서 객체 생성
...
# 상속: 파이썬에서는 별도의 키워드 없이 클래스 정의 시 매개변수 형태로 상속한다.
...
print("상속 후", moreCal.pow()) # 4의 3승 결과 출력
