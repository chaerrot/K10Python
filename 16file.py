'''
open()
    : 파일을 다룰 때 사용되는 내장함수로 첫 번째 인자 file경로만 필수인자임.
    형식]
    ...
    mode: 파일 열기 모드
        ...
        b: 2진모드. 바이너리모드라고도 하는데, 컴퓨터가 알아볼 수 있는
            파일을 생성한다.
...
while True:
    line = f_read.readline() # 파일 내용 한 줄을 읽는다.
...
# 한 줄을 추가한다. 개행문자가 없어 줄바꿈 처리가 되지 않는다.
f_add.write("추가하는 내용입니다.")
# 리스트를 통해 여러 줄의 내용을 입력한다.
...
# kosmo.p 라는 파일을 2진파일 쓰기 모드로 오픈한다.
...
# 2진파일 읽기 모드로 파일을 오픈한 후 load()를 통해 복원한다.
