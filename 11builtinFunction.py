...
'''
print("="*5, "sorted()", "="*20)
numbers = (8,7,6,5,4,9,7,5,3,2,7,4,9,8,2,6,5) #튜플로 데이터를 생성
...
제너레이터 함수
    ...
    그리고 return문 대신 tield문을 사용하여 반복 가능한 객체를 생성한다.
'''
print("="*5, "제너레이터 함수1", "="*20)
print('yield의 동작 과정')
def number_generator1(): # 제너레이터 함수 정의
        yield 0 # 첫 번째 next()를 통해 0을 함수 밖으로 반환(양보)한다.
...
'''
for문을 통해 반복할 때마다 next()를 자동으로 호출하므로
...
def upper_generator(val):
    for i in val:
        yield i.upper() # 리스트의 원소를 대문자로 변경한 후 함수 밖으로 반환한다. 
fruits = ['apple', 'pear', 'grape', 'pineapple', 'orange']
for i in upper_generator(fruits): # 리스트의 인자만큼 반복 호출된다.
    print(i, end=' ')
# 실행결과: 리스트의 문자열이 모두 대문자로 출력된다.