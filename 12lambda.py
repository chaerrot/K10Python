...
def two_sum(x,y):
    return x + y
print("함수를 통합 두 수의 합= ", two_sum(10, 20))
...
map()함수
    ...
    한다. for문과 같은 반복문을 사용하지 않아도 저장한 함수로 인수를
    여러 번 전달해서 그 결과를 List형태로 반환하는 유용한 함수이다.
    형식]
        map(람다식, 파라미터);
'''
print('### 람다식과 map함수1 ###')
...
# 인자가 3의 배수인 경우 문자 형태로 반환하고, 아니면 숫자 그대로를 반환하는 람다식 정의
...
# 50 초과 & 1000 미만인 경우에만 true를 반환하는 람다식을 통해 원소를 필터링한다.
