'''
전역변수와 지역변수
    : 파이썬에서느 변수 선언 시 자료형을 별도로 기술하지 않으므로
...
1. 순서매개변수: 함수에서 사용하는 일반적인 매개변수로 작성 순서대로 전달된다.
'''
...
# 만약 Java에서의 호출이라면 첫 번째 호출에서 에러가 발생하게 될 것이다.

'''
4. 가변 매개변수
    : 여러 개의 매개변수를 전달해야할 때 사용하는 매개변수로
    Java의 가변인자와 비슷하다.
